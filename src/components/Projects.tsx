...
const PROJECTS = [
  ...
  {
    title: "Auralis",
    description: "Real-time acoustic drone detection system that identifies UAV presence from audio alone — built for passive perimeter security where cameras and radar fail. CNN trained on 180K+ mel-spectrograms, exported to ONNX (288KB quantized) with 0.12ms inference. Investigated and validated against data leakage and padding artifacts.",
    link: "https://auralis-acoustic-drone-detector.vercel.app/",
    github: "https://github.com/sara-2-ai-ml/Auralis---acoustic-drone-detector",
    image: "/auralis.png",
    tags: ["PYTHON", "PYTORCH", "CNN", "ONNX", "LIBROSA", "EDGE AI"]
  }
];
          ...
          {PROJECTS.map((project, index) => {
            ...
            >
              <a
                ...
                className={`group relative w-full block transition-transform duration-500 overflow-hidden ${hasLiveLink ? "hover:-translate-y-2 cursor-pointer" : "cursor-default"}`}
                onClick={(e) => {
                  if (!hasLiveLink) e.preventDefault();
                }}
              >
                {hasLiveLink && (
                ...
                {project.image ? (
                  <Image
                    ...
                    width={1600}
                    height={900}
                    className={`w-full h-auto object-contain transition-transform duration-700 border border-[rgba(26,10,6,0.08)] rounded-lg ${hasLiveLink ? "group-hover:scale-[1.02]" : ""}`}
                    onError={(e) => {
